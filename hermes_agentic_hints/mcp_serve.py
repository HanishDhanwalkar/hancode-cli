"""
Agentic framework MCP server — expose lightweight conversation/session tools.

Starts a stdio MCP server that lets any MCP client (Claude Code, Cursor, Codex,
etc.) list conversations, read message history, send messages, poll for live
events, and manage approval requests across all connected platforms.

Matches the lightweight session bridge surface:
...
Usage:
    python -m hermes_agentic_hints.mcp_serve
    python -m hermes_agentic_hints.mcp_serve --verbose
...
MCP client config (e.g. claude_desktop_config.json):
    {
        "mcpServers": {
            "agentic-hints": {
                "command": "python",
                "args": ["-m", "hermes_agentic_hints.mcp_serve"]
            }
        }
    }
"""
...
except ImportError:  # pragma: no cover - optional dependency in starter mode
    _MCP_SERVER_AVAILABLE = False

    class FastMCP:  # type: ignore[no-redef]
        """Small FastMCP-compatible collector for local, dependency-free tests.

        This is not a protocol server. ``run_mcp_server`` still requires the
        real fastmcp package, but the fallback lets us instantiate the bridge
        and call registered tool handlers in unit/integration tests.
        """
        ...
        async def run_stdio_async(self):  # pragma: no cover - defensive guard
            raise RuntimeError("fastmcp is required to run the stdio MCP server")

try:
    from .config import MCPName
...
logger = logging.getLogger("agentic_hints.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory for the local project home."""
    return get_proj_home() / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        directory_file = get_proj_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("AGENTIC_HOME")
            or (Path.home() / ".agentic_hints")
...
class EventBridge:
    ...
    This is the local equivalent of a WebSocket gateway bridge.
    ...
    def __init__(self):
        ...
        self._session_archives_mtime: float = 0.0
    ...
    def _poll_loop(self):
        ...
        if not db:
            logger.warning(
                "EventBridge: SessionDB unavailable, event polling disabled")
            return
    ...
    def _poll_once(self, db):
        ...
        db_file = get_proj_home() / "state.db"
        ...
        archive_mtime = 0.0
        for entry in self._cached_sessions_index.values():
            session_id = entry.get("session_id", "")
            if not session_id:
                continue
            archive_path = get_proj_home() / "sessions" / f"{session_id}.json"
            try:
                archive_mtime = max(archive_mtime, archive_path.stat().st_mtime)
            except OSError:
                continue

        if (
            not sessions_changed
            and db_mtime == self._state_db_mtime
            and archive_mtime == self._session_archives_mtime
        ):
            return  # Nothing changed since last poll — skip entirely

        self._state_db_mtime = db_mtime
        self._session_archives_mtime = archive_mtime
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the MCP server with all tools registered."""

    mcp = FastMCP(
        MCPName,
        instructions=(
            "Local agentic bridge for session archives and channel metadata."
        ),
    )
    ...
    def conversations_list(
        ...
        for key, entry in entries.items():
            ...
            entry_platform = entry.get(
                "platform") or origin.get("platform", "")
    ...
    def events_poll(
        ...
        """
        after_cursor = _coerce_int(
            after_cursor, default=0, minimum=0, maximum=10**18)
...
def run_mcp_server(verbose: bool = False) -> None:
    ...
    if not _MCP_SERVER_AVAILABLE:
        print(
            "Error: MCP server requires the 'fastmcp' package.\n"
            f"Install with: {sys.executable} -m pip install fastmcp",
            file=sys.stderr,
        )
        sys.exit(1)
...
def main(argv: Optional[List[str]] = None) -> None:
    """CLI entrypoint for ``python -m hermes_agentic_hints.mcp_serve``."""
    import argparse

    parser = argparse.ArgumentParser(
        description="Run the hermes_agentic_hints MCP server over stdio."
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
