...
try:
    ...
    from agent.model_metadata import fetch_model_metadata, MINIMUM_CONTEXT_LENGTH, estimate_messages_tokens_rough, get_model_context_length
...
Summary generation was unavailable, so this is a best-effort deterministic fallback for {len(turns_to_summarize)} compacted message(s).{reason_text}"""
        summary = self._with_summary_prefix(body.strip())
