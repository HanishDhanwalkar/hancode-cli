...
def _normalized_custom_base_url(value: str) -> str:
...
def _custom_provider_extra_body_for_agent(
    ...
    if not target_url or (provider or "").strip().lower() != "custom":
        return None

    fallback: Optional[Dict[str, Any]] = None
    for entry in custom_providers or []:
        if not isinstance(entry, dict) or _normalized_custom_base_url(entry.get("base_url")) != target_url:
            continue
        extra_body = entry.get("extra_body")
        if not isinstance(extra_body, dict) or not extra_body:
            continue
        provider_model = str(entry.get("model", "") or "").strip().lower()
        if provider_model == str(model or "").strip().lower() or not provider_model and not fallback:
...
def _merge_custom_provider_extra_body(agent, custom_providers: List[Dict[str, Any]]) -> None:
    ...
    overrides = agent.request_overrides or {}
...
def init_agent(
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            ) or "") if _is_native_anthropic else (api_key or "")
                ...
                if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                    ...
                    for _fb in _fb_entries:
                        ...
                        if not _fb_explicit_key:
                            _fb_key_env = (_fb.get("key_env") or _fb.get(
    ...
    if agent.tools:
        ...
        if not agent.quiet_mode:
            ...
            if disabled_toolsets:
                print(
                    f"   ❌ Disabled toolsets: {', '.join(disabled_toolsets)}")
    elif not agent.quiet_mode:
        print("🛠️  No tools loaded (all tools filtered out or unavailable)")

    # Kanban worker/orchestrator lifecycle guidance is session-static:
    # the dispatcher decides at spawn time whether this process is a kanban
    # worker (kanban_show tool is present iff AGENTIC_KANBAN_TASK is set).
    ...
    try:
        ...
        os.environ["AGENTIC_SESSION_ID"] = agent.session_id

    # Session logs go into the project home alongside other local runtime state.
    project_home = get_proj_home()
    agent.logs_dir = project_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (project_home/sessions/session_{sid}.json)
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    _init_kwargs = {
                        ...
                        "project_home": str(get_proj_home()),
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "agentic"
                    except Exception:
                        pass
    ...
    agent._environment_probe = bool(
        _agent_section.get("environment_probe", True))
    ...
    try:
        ...
        pass
    compression_enabled = str(_compression_cfg.get(
        "enabled", True)).lower() in {"true", "1", "yes"}
    ...
    try:
        _ctx_cfg = _agent_cfg.get("context", {}) if isinstance(
            _agent_cfg, dict) else {}
