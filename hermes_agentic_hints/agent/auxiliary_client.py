...
This starter version is intentionally local-first. It keeps a small
OpenAI-compatible client path for Ollama and other localhost-compatible
servers, while leaving the broader provider-routing machinery in place as
fallback scaffolding for future expansion.

Default ``auto`` resolution prefers a local Ollama-style endpoint, then
falls back to any explicitly configured OpenAI-compatible base URL.

Per-task overrides can still be supplied by the caller via ``main_runtime``
or direct arguments, but the happy path is a local model with no auth.
"""
...
class _LocalCompletionResponse:
    """Small response object matching the OpenAI chat-completions shape."""

    def __init__(self, payload: Dict[str, Any]):
        self.id = payload.get("id")
        self.model = payload.get("model")
        self.created = payload.get("created")
        self.object = payload.get("object", "chat.completion")
        self.choices = []
        ...
        self.usage = SimpleNamespace(**usage) if isinstance(usage, dict) else SimpleNamespace()


class _LocalChatCompletions:
    def __init__(self, client: "_BaseLocalOpenAIClient"):
        self._client = client
    ...
    def create(self, **kwargs):
        ...
        headers = dict(self._client.default_headers)
        if self._client.api_key:
            headers.setdefault("Authorization", f"Bearer {self._client.api_key}")
        with httpx.Client(timeout=timeout, headers=headers) as client:
            response = client.post(url, json=payload)
            response.raise_for_status()
            return _LocalCompletionResponse(response.json())


class _LocalAsyncChatCompletions:
    def __init__(self, client: "_BaseLocalAsyncOpenAIClient"):
        self._client = client
...
class _BaseLocalOpenAIClient:
    def __init__(self, api_key: str = "", base_url: str = "", timeout: float | None = None, default_headers: Optional[Dict[str, str]] = None):
...
class _BaseLocalAsyncOpenAIClient:
    def __init__(self, api_key: str = "", base_url: str = "", timeout: float | None = None, default_headers: Optional[Dict[str, str]] = None):
        self.api_key = api_key or "no-key-required"
        self.base_url = base_url or "http://localhost:11434/v1"
        self.timeout = timeout
        self.default_headers = dict(default_headers or {})
        self.chat = SimpleNamespace(completions=_LocalAsyncChatCompletions(self))


def _load_openai_cls() -> type:
    """Import and cache ``openai.OpenAI`` or fall back to a local client."""
    global _OPENAI_CLS_CACHE
    if _OPENAI_CLS_CACHE is not None:
        return _OPENAI_CLS_CACHE
    try:
        from openai import OpenAI as _cls
        _OPENAI_CLS_CACHE = _cls
    except ImportError:
        _OPENAI_CLS_CACHE = _BaseLocalOpenAIClient
    return _OPENAI_CLS_CACHE
...
try:
    from ..config import get_proj_home
except ImportError:
    from config import get_proj_home
...
def _resolve_custom_runtime() -> Tuple[Optional[str], Optional[str], Optional[str]]:
    """Resolve the active OpenAI-compatible runtime from env/runtime state."""
    openai_base = (
        _RUNTIME_MAIN_BASE_URL
        or os.getenv("OLLAMA_HOST", "")
        or os.getenv("OPENAI_BASE_URL", "")
    ).strip().rstrip("/")
    openai_key = (_RUNTIME_MAIN_API_KEY or os.getenv("OPENAI_API_KEY", "")).strip()
    api_mode = (_RUNTIME_MAIN_API_MODE or "").strip() or None
    if not openai_base:
        return None, None, None
    if not openai_base.endswith("/v1"):
        openai_base = openai_base.rstrip("/") + "/v1"
    if not openai_key:
        openai_key = "no-key-required"
    return openai_base, openai_key, api_mode
...
def _looks_like_local_base_url(base_url: str) -> bool:
    """Return True for loopback/private/local endpoints."""
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if isinstance(sync_client, _BaseLocalOpenAIClient):
        return _BaseLocalAsyncOpenAIClient(
            api_key=sync_client.api_key,
            base_url=str(sync_client.base_url),
            timeout=sync_client.timeout,
            default_headers=sync_client.default_headers,
        ), model
    ...
    try:
        ...
        return _BaseLocalAsyncOpenAIClient(**async_kwargs), model
...
def resolve_provider_client(
    ...
    if isinstance(main_runtime, dict):
        runtime_base = str(main_runtime.get("base_url", "") or "").strip()
    ...
    # Local-first shortcut: Ollama, llama.cpp, LM Studio, and other
    # OpenAI-compatible local servers should resolve before the legacy
    # provider-auth branches below.
    if provider in {"auto", "custom", "local", "ollama"} or _looks_like_local_base_url(local_base):
        resolved_base = local_base or "http://localhost:11434"
        if not resolved_base.endswith("/v1"):
            resolved_base = resolved_base.rstrip("/") + "/v1"
        resolved_key = local_key or "no-key-required"
        resolved_model = _normalize_resolved_model(
            model or _get_aux_model_for_provider("ollama") or _read_main_model() or "llama3.2",
            "ollama",
        )
        client = OpenAI(api_key=resolved_key, base_url=resolved_base)
        return (_to_async_client(client, resolved_model, is_vision=is_vision) if async_mode
                else (client, resolved_model))
